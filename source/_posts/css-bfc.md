...
容器上有 `transform: translateZ(0)`，里面的 `position: fixed` 弹窗表现异常——因为 transform 除了创建层叠上下文，还会成为 fixed 后代元素的包含块，fixed 从此不再相对视口定位。别为了「GPU 加速」随手给祖先元素加 transform。
...
它会创建 BFC，于是高度塌陷、margin 折叠、浮动环绕一起消失——「诶，好了」。但它同时会裁剪溢出内容：dropdown、tooltip、popover 超出父级边界的部分直接不见——这类「元素消失了」的问题，九成不是 z-index，而是被某个祖先的 overflow 裁掉了。我的原则：要 BFC 就写 `flow-root`，真的要裁剪才写 `overflow: hidden`。
...
- **层叠上下文的比较规则？** z-index 只在同一层叠上下文内比较；上下文之间由各自的 z-index 分胜负，子上下文整体跟随父级，出不了自家那层楼。
...
写到这里再收一下：BFC 管「怎么布局」，层叠上下文管「谁在上面」——前者是房间，后者是楼层。楼层的胜负在楼层之间决定，房间里的 z-index 再大，也出不了自家那层楼。把这两个模型放在脑子里的好处是，下次再撞上「莫名其妙」的样式问题，至少知道从哪一层开始查。
