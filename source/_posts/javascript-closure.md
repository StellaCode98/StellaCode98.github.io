...
description: 闭包常被记成「函数记住外部变量」这句口诀，但它的实质是词法作用域与引用关系的结果：函数创建时就保存了定义处的词法环境，外部函数执行完毕后，该环境因被内部函数引用而保留。本文从作用域链讲到闭包的形成原理、引用而非快照、数据私有化与模块模式，再到循环陷阱与内存注意点。
categories:
  - [前端基础, JavaScript]
tags:
  - JavaScript
...
闭包是 JavaScript 里被讨论得最多的概念之一。它看起来像是「函数记住了外部变量」，但只记住这句话，遇到实际代码还是容易疑惑。

先把我的理解放在前面：闭包 = 函数 + 它定义时所处词法环境的引用。外部函数执行结束后，只要内部函数还在被使用，这个环境就不会被回收。要说清它，得先理解作用域、作用域链以及函数执行时的变量查找过程。
...
`increment` 不仅还能访问 `count`，还能持续修改它——三次调用共享同一个 `count`：
...
闭包不是孤立的语法特性，它建立在 JavaScript 的词法作用域之上——变量的可见性由代码书写的位置决定，而不是由调用的位置决定。
...
`inner` 打印的是 `outer`，因为它查找 `name` 时，沿着定义时的作用域关系一层层向外找：
...
这条逐层向外的查找路径就是作用域链。再换个角度验证一下它的方向性——内层可以访问外层，反过来不行：
...
`outer` 永远看不到 `inner` 内部的变量，因为作用域链的方向是内层 → 外层，变量只能「向上」查找，不能「向下」穿透。

记住这一点，闭包的原理就讲完一半了：内部函数从创建起就持有通向外层作用域的查找路径，不管之后在哪里被调用。
...
**第 3 步：`createCounter` 执行结束。** 它的执行上下文出栈了，但它的词法环境不会销毁——因为还有一条从 `counter` 出发、经由内部函数指向它的引用链。所以 `counter()` 依然可以读写 `count`。

所以不要把闭包理解成什么特殊的语法，它就是函数与其词法作用域之间的一种持续引用关系。

## 四、为什么外部函数结束了，变量还活着

这是初学闭包时最容易困惑的地方：「函数都执行完了，局部变量不是应该销毁吗？」

答案藏在垃圾回收的判定规则里。JavaScript 的垃圾回收只关心一件事：对象是否仍然可达。
...
只要这条链还在，`count` 就一直活着。换句话说，并没有什么「闭包强行留住变量」的特殊机制，只是「只要有对象还被引用着，垃圾回收就不动它」这条通用规则，恰好覆盖了被闭包引用的变量。

## 五、闭包捕获的是变量本身，不是快照

一个常被忽略、但解释了很多现象的细节：闭包保存的是变量的引用，而不是创建函数那一刻的值拷贝。
...
`getX` 拿到的永远是 `x` 当前的值。理解了这一点，下一节的经典循环陷阱就不再神秘——它本质上就是「三个回调共享同一个变量的引用」。
...
`count` 没有作为属性暴露出去，`counter.count = 100` 这样的修改是无效的——外部只能通过 `increment` / `decrement` / `getCount` 操作它。这就是数据私有化：数据只能通过受控的方法访问。
...
原因正是第五节说的「捕获引用而非快照」：`var` 声明的 `i` 属于函数作用域，整个循环只有一个 `i`，三个回调闭包引用的是同一个变量：
...
一个常见的误区要先澄清：闭包不等于内存泄漏。闭包确实会延长变量的生命周期，但「被闭包引用所以不回收」和「该回收却没回收」是两回事，垃圾回收关注的始终是可达性。
...
所以结论是：闭包本身不是内存泄漏；错误地长期持有闭包引用（尤其是引用了大对象的闭包），才可能导致内存无法及时释放。

## 九、如何真正理解闭包：三个层次

**第一层：现象。** 函数可以访问定义位置外部的变量，即使外部函数已经执行结束。

**第二层：原因。** JavaScript 使用词法作用域，函数在创建时就通过 `[[Environment]]` 保存了定义处的词法环境；内部函数无论之后被传到哪里调用，都沿着这条引用找回外部变量。
...
最后把闭包的定义收拢一下：闭包就是函数与其词法作用域之间形成的持续引用关系，使函数即使离开原来的执行环境，也仍然能够访问其中的变量。
...
闭包真正有价值的地方就在保存状态、控制数据访问、延长变量生命周期这三件事上。

所以闭包并不是 JavaScript 里某个「特殊的语法」，而是作用域、函数以及引用关系共同产生的一种语言机制。从这角度理解它之后，很多看起来独立的问题——防抖、节流、循环中的异步回调、数据私有化、工厂函数——其实都能归结到同一个概念上。
