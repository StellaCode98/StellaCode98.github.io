...
先立地基：JS 单线程，同一时刻调用栈里只有一段代码在跑，同步代码执行到栈空为止。「异步」只是把事情推迟出去，并非并行。推迟出去的部分由宿主收着（定时器、DOM 事件、网络都不在 JS 线程上干等），时机一到，宿主把回调包装成一个任务（task，俗称宏任务）放进任务队列，事件循环按固定算法一个个调回 JS 线程：
...
整段脚本是第一个宏任务：同步执行到末尾，`then` 回调入微任务队列、`setTimeout` 回调入宏任务队列；脚本任务结束后先清微任务（`promise`），下一轮循环才取宏任务（`timeout`）。

### 坑 1：「宏任务队列」其实不止一条

任务队列按任务源分多条（定时器一条、DOM 事件一条、网络一条），每轮从哪个源取由浏览器决定。所以「宏任务 vs 微任务」真正的区别在调度方式，「两个队列谁在前」反而说不清：宏任务一轮取一个、任务之间可插渲染；微任务每个宏任务之后必清空。后面所有的「玄学顺序」都是这条的推论。
...
拆解：`a()` 同步执行到第一个 await——打印 `a-1`，`b()` 也是同步调用打印 `b`；`a-2` 及之后的部分作为「b 结果的 then 回调」进微任务队列；同步代码继续打印 `end`；清微任务：`then-1`（执行后挂出 `then-2`）→ `a-2` → `then-2`；最后宏任务 `timeout`。记法就一条：await 后面的代码约等于写在 then 里的代码——在哪个 await 暂停，就从那里进微任务队列。
...
最终输出：**1 → 8 → 4 → 7 → 6 → 2 → 3 → 5**。推的时候只问三个问题：栈空了吗？微任务队列还有吗？宏任务该取哪一个了？——比任何口诀都可靠。
...
事件回调是一整个宏任务，而样式计算、布局、绘制都发生在任务之间——浏览器只看得到这个任务的最终状态（`none`），中间那次「显示」从未被画出来。修复的关键是把任务切开，让一帧真正提交：
...
rAF 的位置比较特殊：回调跑在「更新渲染」这一步里、样式计算和布局之前，不走普通宏任务的路径，专为「绘制前最后一刻改样式」设计。这也是双 rAF 的原理：只等一帧不够，因为第一个 rAF 回调执行时那一帧还没画。
...
微任务清空之前没有渲染机会，这个循环让事件循环永远出不了当前轮次，页面直接冻结；换成 `setTimeout(loop, 0)` 就没事。这里能引出一个容易被忽略的结论：真正能让出 UI 的是宏任务，光靠异步让不出来——微任务让得再频繁，渲染和输入响应一帧都等不到。
...
超过 50ms 的同步任务会明显伤害输入响应和帧率（RAIL 模型的经验值）。渲染只发生在任务之间，标准解法就是主动切：
...
注意和 `await Promise.resolve()` 的区别：那样根本没让出——微任务清空之前既不渲染也不响应输入。让出主线程就是进入下一个宏任务，这句是本节的全部内容。
...
最后整个机制可以收成一条主线：同步代码一口气跑完 → 清空微任务 → 也许渲染 → 下一个宏任务。浏览器如此；Node 只是换了一套宏任务的来源（六个阶段）和两条更早清空的队列（nextTick、微任务），主循环的规则不变。
