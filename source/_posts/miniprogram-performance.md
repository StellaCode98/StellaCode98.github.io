...
三板斧各治一段：分包治下载，骨架屏治感知，长列表治运行时。本文以微信小程序为例，每条给结论和能直接抄的代码。动手前建议先跑一遍开发者工具的「体验评分」面板，让数据告诉你卡在哪一段。
...
小程序要先下载主包、再渲染首页，主包越大，用户看到的第一个像素越晚。硬性限制：

```text
单个分包 / 主包 ≤ 2M        整个小程序（所有分包合计）≤ 30M
```

主包超 2M 直接没法上传，所以分包没什么好犹豫的。
...
- **independent（独立分包）**：不依赖主包、单独运行。用户从分享卡片直接进活动页时完全不下载主包，秒开——适合活动、落地页
...
感知性能往往比真实性能更值钱：白屏超过 300ms 就该有骨架屏。两条底线：
...
偷懒方案：开发者工具模拟器面板「…」菜单里有一键生成骨架屏，会按当前页面结构生成 `skeleton.wxml/wxss`，适合快速上线；结构精细的页面建议手写，可控得多。
...
小程序逻辑层（JS）和渲染层（WebView）是两个线程，`setData` 的数据要先序列化，再通过桥接传到另一个线程：
...
所以优化方向很直白：让每次 setData 传输的数据尽量小、调用频率尽量低。
...
信息流里一屏只见 3~4 条，用户往下滚才需要后面的内容。用 `IntersectionObserver` 只渲染进入视口的 item，DOM 数量能砍到 1/10：
...
限制要知道：`recycle-view` 要求条目定高（不定高的列表要先估算高度），这是它拿灵活性换性能的代价。几百条以内的列表，用 3.2~3.4 的组合就够了，不必上虚拟列表。
...
落地顺序上，个人建议：先跑体验评分定位阶段，分包保启动（硬性要求），然后骨架屏保感知（半天工作量），再做分页 + 差量 setData（性价比最高），真有万级列表才上 recycle-view。
